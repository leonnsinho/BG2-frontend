  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-[#EBA500]/5 to-transparent">
          <div className="flex items-center gap-3">
            ...
            {editingLead ? (
              <input
                className="text-lg font-bold text-gray-900 border-b-2 border-[#EBA500] bg-transparent outline-none px-1"
                value={leadForm.nome_empresa}
                onChange={e => setLeadForm(p => ({ ...p, nome_empresa: e.target.value }))}
                autoFocus
              />
            ) : (
              <h1 className="text-xl font-bold text-gray-900">{lead.nome_empresa}</h1>
            )}
          </div>
          <div className="flex items-center gap-2">
            {editingLead ? (
              <>
                <button onClick={() => setEditingLead(false)} className="px-3 py-1.5 text-sm text-gray-600 border border-gray-200 rounded-xl hover:bg-gray-50">Cancelar</button>
              ...
              <button onClick={() => setEditingLead(true)} className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-600 border border-gray-200 rounded-xl hover:bg-gray-50">
          ...
          {showContactForm && (
            <div className="mb-4 p-4 bg-gray-50 rounded-xl border border-gray-200 space-y-3">
              ...
              <div className="flex justify-end gap-2">
                <button onClick={() => setShowContactForm(false)} className="px-4 py-2 text-sm text-gray-600 border border-gray-200 rounded-xl hover:bg-gray-100">Cancelar</button>
              ...
              {contacts.map(c => (
                <div key={c.id} className="flex items-center gap-3 p-3 border border-gray-100 rounded-xl hover:bg-gray-50 group">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-800">{c.nome}{c.cargo ? <span className="font-normal text-gray-500"> · {c.cargo}</span> : ''}</p>
