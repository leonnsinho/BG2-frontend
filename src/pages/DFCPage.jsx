...
function DFCPage() {
  ...
  useEffect(() => {
    const loadItensEmpresa = async () => {
      ...
      try {
        ...
        console.log(`📋 DFC SAÍDAS - Itens carregados: ${itensFiltrados.length} (empresa: ${idsEmpresa.size}, globais: ${itensFiltrados.length - idsEmpresa.size})`)
  ...
  const fetchCategoriasEItens = async () => {
    try {
      ...
      if (!isSuperAdmin() && userCompanyId) {
        ...
        console.log(`📋 DFC CARGA INICIAL - Itens filtrados: ${itensFiltrados.length} de ${itensData?.length || 0}`)
      }
    ...
    try {
      ...
      if (editingId) {
        setSaidas(prev => prev.map(s => {
          ...
          if (s.lancamento_pai_id === editingId) {
            return { ...s, categoria: formData.categoria, item_id: formData.item_id }
          }
          return s
        }))
      } else {
  ...
  const handleSaveParcela = async (parcelaId) => {
    try {
      ...
      const paiId = selectedSaidaParcelas?.id || editingId
      ...
      setParcelas(parcelasData)
      if (selectedSaidaParcelas) {
        setSelectedSaidaParcelas({ ...selectedSaidaParcelas, valor: valorTotal })
      }
      if (editingId) {
      ...
      setSaidas(prev => prev.map(s => s.id === paiId ? { ...s, valor: valorTotal } : s))
  ...
  const filteredSaidas = useMemo(() => {
    ...
    return saidas.filter(saida => {
      ...
      const matchEspecie = !especieFilter || !!saida.dinheiro_especie
  ...
  }, [saidas, searchTerm, mesFilter, dataInicio, dataFim, sortCol, sortDir, especieFilter])
                ...
                <div className="border border-gray-200 dark:border-gray-600 rounded-xl p-4 bg-amber-50/40 dark:bg-amber-900/10">
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Marque se esta saída foi paga em dinheiro físico</p>
                    </div>
                  </label>
                </div>
                          ...
                          >
                            ...
                            <div className="ml-3 flex items-center space-x-2 shrink-0">
                              {editingParcelaId === parcela.id ? (
                                ...
                                <p className="text-sm font-bold text-gray-900 dark:text-white">{formatCurrency(parcela.valor)}</p>
                              )}
                              {editingParcelaId === parcela.id ? (
