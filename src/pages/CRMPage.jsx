        ...
        <div className="flex-shrink-0 px-6 py-4 border-b border-gray-100">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <>
              ...
              <button onClick={() => { onClose(); navigate('/crm/contatos') }} className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-purple-600 bg-purple-50 hover:bg-purple-100 border border-purple-200 rounded-xl transition-colors">
...
function ProductsModal({ companyId, onClose }) {
  ...
  const parseProductsFile = async (file) => {
    ...
    if (isXlsx) {
      ...
      const ws = wb.Sheets[wb.SheetNames[0]]
      const data = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' })
      return data.slice(1).filter(row => row.some(cell => String(cell).trim())).map(row => row.map(cell => String(cell ?? '').trim()))
    } else {
      const text = await file.text()
      return text.split('\n').slice(1).filter(l => l.trim()).map(l => l.split(',').map(c => c.trim().replace(/^"|"$/g, '')))
    }
  }
  ...
  const handleImportFile = async (e) => {
    ...
    try {
      ...
      if (preview.length === 0) { toast.error('Nenhum produto válido encontrado'); return }
  ...
  const confirmImportProducts = async () => {
    ...
    try {
      ...
      toast.success(`${inserted} produto${inserted !== 1 ? 's' : ''} importado${inserted !== 1 ? 's' : ''}` + (errors ? ` (${errors} erro${errors !== 1 ? 's' : ''})` : ''))
      setImportPreview(null)
      loadProducts()
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] flex flex-col">
        <div className="flex-shrink-0 px-6 py-4 border-b border-gray-100">
          ...
          {!showForm && (
            <div className="flex flex-wrap items-center gap-2">
              <button onClick={() => importRef.current?.click()} disabled={importing} className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-green-700 bg-green-50 hover:bg-green-100 border border-green-200 rounded-xl transition-colors disabled:opacity-60" title="Importar produtos de planilha CSV ou Excel">
                <FileSpreadsheet className="h-3.5 w-3.5" /> Importar planilha
              </button>
              <button onClick={downloadTemplateXlsx} className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-xl transition-colors" title="Baixar modelo Excel">
                <Download className="h-3.5 w-3.5" /> Modelo .xlsx
              </button>
              <button onClick={downloadTemplateCsv} className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-xl transition-colors" title="Baixar modelo CSV">
                <Download className="h-3.5 w-3.5" /> Modelo .csv
              </button>
              <button onClick={openNew} className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-white bg-[#EBA500] hover:bg-[#d49500] rounded-xl transition-colors">
...
// ─── ContactsModal ────────────────────────────────────────────────────────────
function ContactsModal({ companyId, onClose, onOpenLeads }) {
  const { user } = useAuth()
  const [contacts, setContacts] = useState([])
  const [leads, setLeads] = useState([])
  const [loading, setLoading] = useState(true)
  ...
  const handleImportFile = async (e) => {
    ...
    try {
      ...
      if (isXlsx) {
        ...
        const wb = XLSX.read(buffer, { type: 'array' })
        const ws = wb.Sheets[wb.SheetNames[0]]
        rows = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' }).slice(1).filter(r => r.some(c => String(c).trim())).map(r => r.map(c => String(c ?? '').trim()))
      } else {
        const text = await file.text()
        rows = text.split('\n').slice(1).filter(l => l.trim()).map(l => l.split(',').map(c => c.trim().replace(/^"|"$/g, '')))
      }
      const preview = rows.filter(r => r[0]?.trim()).map(([nome = '', cargo = '', email = '', telefone = '', empresa = '', observacoes = '']) => ({ nome: nome.trim(), cargo: cargo.trim(), email: email.trim(), telefone: telefone.trim(), empresa: empresa.trim(), observacoes: observacoes.trim() }))
  ...
  const confirmImportContacts = async () => {
    ...
    try {
      ...
      for (const r of importPreview.rows) {
        ...
        const { error } = await supabase.from('crm_contacts').insert([{ nome: r.nome, cargo: r.cargo || null, email: r.email || null, telefone: r.telefone || null, observacoes: r.observacoes || null, lead_id: leadMatch?.id || null, company_id: companyId, created_by: user?.id }])
        if (error) errors++; else inserted++
      }
      toast.success(`${inserted} contato${inserted !== 1 ? 's' : ''} importado${inserted !== 1 ? 's' : ''}` + (errors ? ` (${errors} erro${errors !== 1 ? 's' : ''})` : ''))
      setImportPreview(null)
      loadAll()
    } catch (err) { toast.error('Erro na importação: ' + err.message) }
    finally { setImporting(false) }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] flex flex-col">
        <div className="flex-shrink-0 px-6 py-4 border-b border-gray-100">
          ...
          {!showForm && (
            <div className="flex flex-wrap items-center gap-2">
              <button onClick={() => importRef.current?.click()} disabled={importing} className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-green-700 bg-green-50 hover:bg-green-100 border border-green-200 rounded-xl transition-colors disabled:opacity-60">
                <FileSpreadsheet className="h-3.5 w-3.5" /> Importar planilha
              </button>
              ...
              {onOpenLeads && (
                <button onClick={onOpenLeads} className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-amber-700 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded-xl transition-colors">
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">{importPreview.rows.length} contato{importPreview.rows.length !== 1 ? 's' : ''} encontrado{importPreview.rows.length !== 1 ? 's' : ''}. Revise antes de confirmar.</p>
            </div>
            <button onClick={() => setImportPreview(null)} className="p-2 hover:bg-gray-100 rounded-xl text-gray-400 hover:text-gray-600"><X className="h-4 w-4" /></button>
          </div>
          <div className="flex-1 overflow-auto px-6 py-4">
            <table className="w-full text-xs border-collapse">
              <thead>
          ...
          <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-100">
            ...
            <button onClick={confirmImportContacts} disabled={importing} className="px-5 py-2 text-sm font-semibold text-white bg-[#EBA500] hover:bg-[#d49500] rounded-xl transition-colors disabled:opacity-60 flex items-center gap-2">
              {importing ? <span className="animate-spin h-3.5 w-3.5 border-2 border-white border-t-transparent rounded-full" /> : <Upload className="h-3.5 w-3.5" />}
              Confirmar importação
            </button>
          </div>
        </div>
